...
func (e errUnsafeFloat) Error() string {
	if e.key != "" {
		return fmt.Sprintf("key %q: value %v cannot be represented exactly as %s (round-trip conversion loses precision)", e.key, e.i, e.size)
	}
	return fmt.Sprintf("value %v cannot be represented exactly as %s (round-trip conversion loses precision)", e.i, e.size)
}
