...
// TestOmitEmptyAlignedWithOmitZero verifies that the omitempty struct tag now
// has identical semantics to omitzero: zero-valued integers, floats, bools,
// strings, empty slices/maps, and all-zero structs are all skipped.
func TestOmitEmptyAlignedWithOmitZero(t *testing.T) {
	type S struct {
		Int         int                    `toml:"int,omitempty"`
		Uint        uint                   `toml:"uint,omitempty"`
		Float32     float32                `toml:"float32,omitempty"`
		Float64     float64                `toml:"float64,omitempty"`
		Bool        bool                   `toml:"bool,omitempty"`
		String      string                 `toml:"string,omitempty"`
		Slice       []int                  `toml:"slice,omitempty"`
		Map         map[string]int         `toml:"map,omitempty"`
		NestedZero  struct{ X, Y int }     `toml:"nested_zero,omitempty"`
		NestedValue struct{ X, Y int }     `toml:"nested_value,omitempty"`
		PtrNil      *int                   `toml:"ptr_nil,omitempty"`
		PtrValue    *int                   `toml:"ptr_value,omitempty"`
		IntNonZero  int                    `toml:"int_nonzero,omitempty"`
	}

	nonZero := 42
	in := S{
		// All of these should be OMITTED (they're zero/empty).
		Int:         0,
		Uint:        0,
		Float32:     0.0,
		Float64:     0.0,
		...
		Slice:       nil,
		Map:         nil,
		NestedZero:  struct{ X, Y int }{},
		PtrNil:      nil,
		// These should be PRESENT.
		NestedValue: struct{ X, Y int }{X: 1, Y: 2},
		PtrValue:    &nonZero,
		IntNonZero:  99,
	}
	...
	// Zero/empty fields must not appear in the output.
	omitted := []string{
		"int =", "uint =", "float32 =", "float64 =", "bool =",
		"string =", "slice =", "map =", "nested_zero =", "ptr_nil =",
	}
	for _, prefix := range omitted {
		if strings.Contains(got, prefix) {
			t.Errorf("expected omitempty to skip zero-valued field, but output contains %q\nOutput:\n%s", prefix, got)
		}
	}

	// Non-zero fields must appear.
	present := []string{
		"int_nonzero = 99",
		"ptr_value = 42",
	}
