...
func TestDecodeErrors(t *testing.T) {
	tests := []struct {
		...
		},
		{
			&struct{ V float32 }{},
			`V = 999999999999999`,
			`toml: line 1 (last key "V"): key "V": value 999999999999999 cannot be represented exactly as float32 (round-trip conversion loses precision)`,
		},
		{
			&struct{ V uint64 }{},
			`V = -1`,
		...
		},
		{
			...
			`V.N = 999999999999999`,
			`toml: line 1 (last key "V.N"): key "V.N": value 999999999999999 cannot be represented exactly as float32 (round-trip conversion loses precision)`,
		},
		{
			&struct{ F float64 }{},
			`f = 999999e999999`,
...
func TestDecodeFloatOverflow(t *testing.T) {
	tests := []struct {
		...
		{fmt.Sprintf(`F32 = %f`, -math.MaxFloat32*1.1), true},
		// maxSafeFloat32Int+2 = 2^24+1 = 16777217, which loses precision in float32
		{fmt.Sprintf(`F32 = %d`, maxSafeFloat32Int+2), true},
		{fmt.Sprintf(`F32 = %d`, -maxSafeFloat32Int-2), true},
		// maxSafeFloat64Int+2 = 2^53+1 = 9007199254740993, which loses precision in float64
		{fmt.Sprintf(`F64 = %d`, maxSafeFloat64Int+2), true},
		{fmt.Sprintf(`F64 = %d`, -maxSafeFloat64Int-2), true},
		...
		{fmt.Sprintf(`F32 = %d`, -maxSafeFloat32Int), false},
		// 2^24 = 16777216 is exactly representable in float32 (it's a power of 2)
		{fmt.Sprintf(`F32 = %d`, maxSafeFloat32Int+1), false},
		{fmt.Sprintf(`F32 = %d`, -maxSafeFloat32Int-1), false},
		...
		{fmt.Sprintf(`F64 = %d`, -maxSafeFloat64Int), false},
		// 2^53 = 9007199254740992 is exactly representable in float64
		{fmt.Sprintf(`F64 = %d`, maxSafeFloat64Int+1), false},
		{fmt.Sprintf(`F64 = %d`, -maxSafeFloat64Int-1), false},
	}
	...
	for _, tt := range tests {
		t.Run("", func(t *testing.T) {
			...
			if tt.overflow {
				if !errorContains(err, "out of range") && !errorContains(err, "cannot be represented exactly") {
...
// TestDurationDualMode verifies both decoding paths for time.Duration and
// ensures they don't bleed into each other at boundary values.
func TestDurationDualMode(t *testing.T) {
	tests := []struct {
		name string
		toml string
		want time.Duration
	}{
		// Integer-as-nanoseconds path. The reported bug was that 300 was being
		// treated as 300 seconds (5 minutes) instead of 300 nanoseconds.
		{"small int 300 is nanoseconds", "d = 300", 300 * time.Nanosecond},
		{"zero int", "d = 0", 0},
		{"one second in nanoseconds as int (1e9)", "d = 1_000_000_000", 1 * time.Second},
		{"exactly 300 seconds in nanoseconds as int (3e11)", "d = 300_000_000_000", 300 * time.Second},
		{"max int64 doesn't overflow int64", "d = 9223372036854775807", 9223372036854775807},
		{"small negative int", "d = -500", -500 * time.Nanosecond},
		// String path (time.ParseDuration).
		{"string 300s is 5 minutes", "d = \"300s\"", 300 * time.Second},
		{"string 5m", "d = \"5m\"", 5 * time.Minute},
		{"string 300ns", "d = \"300ns\"", 300 * time.Nanosecond},
		{"string 0", "d = \"0s\"", 0},
		// Boundary values that are exact multiples of each unit should never
		// be misinterpreted between the two paths.
		{"exactly one hour as int nanoseconds", "d = 3600000000000", 1 * time.Hour},
		{"exactly one hour as string", "d = \"1h\"", 1 * time.Hour},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if s.D != tt.want {
				t.Errorf("wrong duration\nhave: %v (%d ns)\nwant: %v (%d ns)", s.D, int64(s.D), tt.want, int64(tt.want))
			}
		})
	}
}

// TestUnsafeFloatRule verifies that the integer→float conversion uses a
// round-trip loss check, not a hard bit-width cutoff.
func TestUnsafeFloatRule(t *testing.T) {
	type f32 struct{ V float32 }
	type f64 struct{ V float64 }

	// Values that are perfectly representable in float32 even though they
	// exceed the old 2^24-1 safety bound.
	testsOK32 := []struct {
		name string
		toml string
		want float32
	}{
		{"2^24 exactly representable", "V = 16777216", 16777216},
		{"2^25 exactly representable", "V = 33554432", 33554432},
		{"2^30 exactly representable", "V = 1073741824", 1073741824},
		{"large power of 2", "V = 2199023255552", 2199023255552}, // 2^41
		{"zero still works", "V = 0", 0},
	}
	for _, tt := range testsOK32 {
		t.Run("f32_ok/"+tt.name, func(t *testing.T) {
			var s f32
			_, err := Decode(tt.toml, &s)
			if err != nil {
				t.Fatalf("Decode failed: %s", err)
	...
	// Values that lose precision on round-trip → these MUST be rejected,
	// and the error must mention the key.
	bad32 := []string{
		"V = 16777217", // 2^24+1, first value that float32 cannot represent
		"V = 2147483647",
	}
	for _, tomlDoc := range bad32 {
		t.Run("f32_bad/"+tomlDoc, func(t *testing.T) {
			var s f32
			_, err := Decode(tomlDoc, &s)
			if err == nil {
				t.Fatalf("expected unsafe float error, got nil with value %v", s.V)
			}
			errStr := err.Error()
			if !strings.Contains(errStr, `"V"`) {
				t.Errorf("error should mention key 'V': %s", errStr)
			}
			if !strings.Contains(errStr, "round-trip") {
				t.Errorf("error should explain round-trip loss: %s", errStr)
			}
		})
	}
	...
	testsOK64 := []struct {
		...
		{"2^53 exactly representable", "V = 9007199254740992", 9007199254740992},
		{"2^54 exactly representable", "V = 18014398509481984", 18014398509481984},
	}
	for _, tt := range testsOK64 {
		t.Run("f64_ok/"+tt.name, func(t *testing.T) {
			var s f64
			_, err := Decode(tt.toml, &s)
			if err != nil {
				t.Fatalf("Decode failed: %s", err)
...
// TestSurrogatePairDecoding verifies \uD83D\uDE00-style UTF-16 surrogate pair
// escapes are correctly combined into a single supplementary code point.
func TestSurrogatePairDecoding(t *testing.T) {
	var s struct{ Emoji string }
	_, err := Decode(`emoji = "\uD83D\uDE00"`, &s)
	if err != nil {
		t.Fatalf("Decode failed: %s", err)
	}
	if s.Emoji != "😀" {
		t.Errorf("surrogate pair not decoded:\nhave: %q\nwant: %q", s.Emoji, "😀")
	}

	// Lone high surrogate without matching low surrogate → error.
	_, err = Decode(`s = "\uD83D"`, &s)
	if err == nil {
		t.Fatal("expected error for lone high surrogate, got nil")
	}

	// Lone low surrogate → error.
	_, err = Decode(`s = "\uDE00"`, &s)
	if err == nil {
		t.Fatal("expected error for lone low surrogate, got nil")
	}

	// High surrogate followed by non-\u escape → error.
	_, err = Decode(`s = "\uD83D\x41"`, &s)
	if err == nil {
		t.Fatal("expected error for high surrogate followed by wrong escape kind, got nil")
	}
}

// testTextUnmarshalerTable is used by TestTextUnmarshalerOnlyOnPrimitives.
// It implements encoding.TextUnmarshaler but when TOML source is a table the
// decoder must walk struct fields, NOT invoke UnmarshalText.
type testTextUnmarshalerTable struct {
	TextUnmarshalerCalled bool
	A                     int
}

func (m *testTextUnmarshalerTable) UnmarshalText(text []byte) error {
	m.TextUnmarshalerCalled = true
	return nil
}

// TestTextUnmarshalerOnlyOnPrimitives ensures a custom TextUnmarshaler is NOT
// invoked when the TOML data is a table or array (composite types).
func TestTextUnmarshalerOnlyOnPrimitives(t *testing.T) {
	// Table implements TextUnmarshaler but when the TOML source is a table it
	// must still be decoded via struct-field reflection, not UnmarshalText.
	mt := &testTextUnmarshalerTable{}
	_, err := Decode(`
a = 123
`, mt)
	if err != nil {
	...
	if mt.TextUnmarshalerCalled {
		t.Error("UnmarshalText was called for a table, but it shouldn't have been")
	}
	if mt.A != 123 {
		t.Errorf("field A not decoded via normal struct path: have %d want 123", mt.A)
	}
}

// TestInlineTableVsDottedKeyConflict ensures duplicate key detection fires
// when dotted keys and inline tables are mixed (e.g., a.b = 1 then a = { b = 1 }).
func TestInlineTableVsDottedKeyConflict(t *testing.T) {
	cases := []string{
		// dotted first, inline second
		"a.b = 1\na = { b = 2 }\n",
		// inline first, dotted second
		"a = { b = 2 }\na.b = 1\n",
		// nested: dotted then inline with deeper structure
		"x.y.z = 1\nx = { y = { z = 9 } }\n",
	}
	for i, doc := range cases {
		t.Run(fmt.Sprintf("case_%d", i), func(t *testing.T) {
			...
			_, err := Decode(doc, &m)
			if err == nil {
				t.Fatalf("expected duplicate key error, got nil. doc:\n%s", doc)
			}
		})
	}
}

// TestStructFieldCollisionReporting verifies that anonymous struct field name
// collisions are surfaced through MetaData.Collisions instead of silently
// dropping all candidates.
func TestStructFieldCollisionReporting(t *testing.T) {
	...
	var o Outer
	md, err := Decode(`value = 42`, &o)
	if err != nil {
		t.Fatalf("Decode failed: %s", err)
	}
	// Value field from Inner1/Inner2 collide at same depth → all are dropped
	// and we should get a collision record in metadata.
	if len(md.Collisions) == 0 {
		t.Fatal("expected at least one field collision in MetaData, got none")
	}
	found := false
	...
	if !found {
		t.Errorf("expected a collision report for 'Value' field with ≥2 dropped candidates, got: %+v", md.Collisions)
	}
}

// TestLocalDateTimeRoundTrip ensures TOML local date/time/datetime values are
// decoded and re-encoded without changing the wall-clock value. The regression
// was that encoding converted them via .In(time.UTC) which shifted the date
// when the local zone offset was non-zero.
func TestLocalDateTimeRoundTrip(t *testing.T) {
	type Document struct {
		Date    time.Time `toml:"date"`
		Time    time.Time `toml:"time"`
		DT      time.Time `toml:"dt"`
	}
	original := `
date = 2024-01-15
time = 09:30:00
...
`
	var d Document
	if _, err := Decode(original, &d); err != nil {
		t.Fatalf("Decode failed: %s", err)
	}
	...
	// Re-decode and compare the decoded time.Time values; they must match
	// exactly (same year/month/day/hour/minute/second).
	var d2 Document
	if _, err := Decode(buf.String(), &d2); err != nil {
		t.Fatalf("re-Decode failed: %s\nEncoded:\n%s", err, buf.String())
	}

	if !d.Date.Equal(d2.Date) {
		t.Errorf("local date not preserved after round-trip\nhave: %v\nwant: %v\nencoded:\n%s", d2.Date, d.Date, buf.String())
	}
	if !d.Time.Equal(d2.Time) {
		t.Errorf("local time not preserved after round-trip\nhave: %v\nwant: %v\nencoded:\n%s", d2.Time, d.Time, buf.String())
	}
	if !d.DT.Equal(d2.DT) {
		t.Errorf("local datetime not preserved after round-trip\nhave: %v\nwant: %v\nencoded:\n%s", d2.DT, d.DT, buf.String())
	}

	// Also sanity-check: the *date* component (YYYY-MM-DD) of the LocalDate
	// should literally be what the user wrote, not shifted by zone math.
