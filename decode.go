...
func (md *MetaData) unify(data any, rv reflect.Value) error {
	...
	for rvK.Kind() == reflect.Pointer {
		...
		rvK = reflect.Indirect(rvK)
		underlyingK = rvK.Kind()
	}
	_, isTableData := data.(map[string]any)
	...
	// We did NOT take the TextUnmarshaler path above. If rv is still a pointer
	// at this point it is because indirect() returned a pointer-to-struct (or
	// pointer-to-map, etc.) to satisfy the TextUnmarshaler check; unwrap it so
	// the type-switch below operates on the concrete value kind.
...
func (md *MetaData) unifyStruct(mapping any, rv reflect.Value) error {
	...
	// Sort keys for stable iteration order so that case-insensitive key
	// matching always picks the same field, and repeated runs produce
	// identical results with the same input.
	...
	result := cachedTypeFieldsResult(rv.Type())
	fields := result.fields
	structName := rv.Type().String()

	// Build a quick lookup for field name collisions in this struct type.
	// key: lowercase field name, value: pointer to collision info
	collisionsByLowerName := make(map[string]*fieldCollision, len(result.collisions))
	for i := range result.collisions {
		c := &result.collisions[i]
		collisionsByLowerName[strings.ToLower(c.name)] = c
	}
	...
	for _, key := range keys {
		...
		if f != nil {
			...
			// No field matched this key. Check if it's because of an
			// anonymous struct field collision that caused all candidates
			// to be silently dropped.
			if c, ok := collisionsByLowerName[strings.ToLower(key)]; ok {
...
func (md *MetaData) unifyFloat64(data any, rv reflect.Value) error {
	...
	if num, ok := data.(int64); ok {
		switch rvk {
		case reflect.Float32:
			if int64(float32(num)) != num {
				return md.parseErr(errUnsafeFloat{i: num, size: rvk.String(), key: md.context.String()})
			}
			rv.SetFloat(float64(float32(num)))
		case reflect.Float64:
			if int64(float64(num)) != num {
				return md.parseErr(errUnsafeFloat{i: num, size: rvk.String(), key: md.context.String()})
			}
